...
작성 원칙 (반드시):
- 맨 앞에 '⭐ 오늘의 핵심 3가지'를 두고, 초보자가 이것만 읽어도 되게 쉽게 쓴다.
...
- 핵심요약표시문이 있으면 ⭐ 섹션에 그 내용을 따른다 (다시 지어내지 마라).
- 📰 뉴스 쓰지 마라. 어제예측검증은 본문 중간·학습 근처 (맨 위 X).
...
⭐ 핵심 3가지 형식 (필수, 쉬운 말):
1. (위쪽 가격)가 가장 중요한 자리 — 왜(콜 거래/OI) + 돌파 여부 확인
2. (아래쪽 가격)는 첫 번째 방어선 — 왜(풋) + 이탈 시 하락 경계
3. 이전 분석의 교훈 — 과거 오해(풋=하락/콜=상승 단정)와 이번 반영점

출력 구조:
📊 오늘의 {티커} 옵션 시장 이야기 - {날짜}
🎯 한 문장 요약
⭐ 오늘의 핵심 3가지 (입력 핵심요약표시문 우선)
💰 가격 (주가표시문 그대로)
🚨 이벤트 (있을 때만)
🌡️ 시장 온도
🟢 지지선 / 🔴 저항선
...
금지: 뉴스, 마크다운 링크, 시나리오 한 줄 합치기, 근거 없는 숫자.
